...
PRINT 'BLOCKER_PFE_SCRIPT_KATMAI Script v10.1.01 (SQL2008)'
PRINT ''
...
PRINT ''
SELECT 'ServerName' AS 'ServerPropertyName', @@SERVERNAME AS 'Value' UNION ALL
SELECT 'PhysicalName', CAST(SERVERPROPERTY('ComputerNamePhysicalNetBIOS') AS VARCHAR) UNION ALL
SELECT 'ProductVersion', CAST(SERVERPROPERTY('ProductVersion') AS VARCHAR) UNION ALL
SELECT 'ProductLevel', CAST(SERVERPROPERTY('ProductLevel') AS VARCHAR) UNION ALL
SELECT 'ResourceVersion', CAST(SERVERPROPERTY('ResourceVersion') AS VARCHAR) UNION ALL
SELECT 'ResourceLastUpdateDateTime', CAST(SERVERPROPERTY('ResourceLastUpdateDateTime') AS VARCHAR) UNION ALL
SELECT 'Edition', CAST(SERVERPROPERTY('Edition') AS VARCHAR) UNION ALL
SELECT 'ProcessId', CAST(SERVERPROPERTY('ProcessId') AS VARCHAR) UNION ALL
SELECT 'SessionId', CAST(@@SPID AS VARCHAR) UNION ALL
SELECT 'Collation', CAST(SERVERPROPERTY('Collation') AS VARCHAR(32)) 
PRINT ''
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_PFE_BEGIN sys.dm_exec_connections_sessions'
...
PRINT 'BLOCKER_PFE_END sys.dm_exec_connections_sessions '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
...
SELECT 
	...
	req.wait_time, CAST(req.wait_type AS VARCHAR(20)) AS 'wait_type', 
...
SELECT
	...
	DATEDIFF(ms, c.creation_time, GETDATE()) AS 'u_elapsed_time', 
...
task_state = CAST(t.task_state AS VARCHAR(10)), 
...
select 
wp.pool_id, CAST(wp.name AS NVARCHAR(16)) AS 'name', wp.statistics_start_time, 
...
SELECT 
wg.group_id, wg.pool_id, CAST(wg.name AS NVARCHAR(16)) AS 'name', 
...
PRINT 'BLOCKER_PFE_END sys.dm_os_process_memory '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
...
SELECT 
	...
	CAST(s.login_name AS NVARCHAR(32)) AS 'login_name', 
	CAST(s.program_name AS NVARCHAR(64)) AS 'program_name', 
	s.host_process_id, 
	c.connection_id,
	CAST(s.original_login_name AS NVARCHAR(32)) AS 'original_login_name', 
	s.client_interface_name, s.client_version, 
	CAST(c.auth_scheme AS VARCHAR(16)) AS 'auth_scheme', 
	CAST(c.net_transport AS VARCHAR(16)) AS 'net_transport', 
	c.client_net_address, c.client_tcp_port, 
...
SELECT database_id FROM sys.databases WHERE has_dbaccess(Name) = 1
